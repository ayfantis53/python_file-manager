...
# File Manager Copy No Protobuf implementation (Windows)
-------------------------------------------------------------------------
...
```bash
# To Check for Errors: Use the ruff check command.
python -m ruff check . ||  make check

# To Automatically Fix Errors:
python -m ruff check --fix . ||  make fix

# To Format Code:
python -m ruff format . ||  make format

# To ensure Ruff is reading the correct file and see exactly what settings are being applied:
python -m ruff config
```
...
```bash
# Run in 1st terminal.
python -m uv run src/utils/server.py
```
...
# Run all tests:
python -m uv run pytest ||  make test

# Run single test file:
python -m uv run pytest tests/test_file_manager.py

# Run single test suite:
python -m uv run pytest tests/test_file_manager.py::TestJSON

# Run single test:
python -m uv run pytest tests/test_file_manager.py::TestJSON::test_json_SUCCESS
```
...
# Run in 2nd terminal.
python -m uv run  src/file_manager.py
```
